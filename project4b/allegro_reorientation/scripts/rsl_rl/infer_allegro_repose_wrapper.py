...
"""Load a TorchScript policy (via RslRlPolicyWrapper) and run a single inference step.
...
env and then runs inference using the exported TorchScript policy through
RslRlPolicyWrapper.
...
parser = argparse.ArgumentParser(description="Infer a single step using a TorchScript policy wrapper.")
parser.add_argument(
    "--task",
    type=str,
...
parser.add_argument("--policy", type=str, default=DEFAULT_POLICY, help="TorchScript policy file to load.")
parser.add_argument(
    "--checkpoint",
    type=str,
    default="PATH/TO/CHECKPOINT.pt",
    help="Optional RSL-RL checkpoint (.pt). If provided, exports TorchScript policy before inference.",
)
parser.add_argument(
    "--export_dir",
    type=str,
    default=None,
    help="Optional export directory for TorchScript policy (defaults to <checkpoint_dir>/exported).",
)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    if args_cli.checkpoint is None and os.path.basename(args_cli.policy).startswith("model_"):
        print("[INFO] Treating --policy as a checkpoint. Set --checkpoint explicitly to silence this.")
        args_cli.checkpoint = args_cli.policy

    # set log dir to policy/checkpoint directory (used by some env utilities)
    base_path = args_cli.policy if args_cli.checkpoint is None else args_cli.checkpoint
    log_dir = os.path.dirname(os.path.abspath(base_path))
    ...
    # If a checkpoint is provided, export TorchScript first.
    policy_path = args_cli.policy
    if args_cli.checkpoint is not None:
        print(f"[INFO] Exporting TorchScript from checkpoint: {args_cli.checkpoint}")
        ...
        export_dir = (
            ...
            else os.path.join(os.path.dirname(args_cli.checkpoint), "exported")
        )
    ...
    # load TorchScript policy via wrapper
    print(f"[INFO] Loading TorchScript policy: {policy_path}")
    ...
    # convert TensorDict -> dict for the policy wrapper
    if hasattr(obs, "to_dict"):
        obs = obs.to_dict()
    assert isinstance(obs, dict) and "policy" in obs, "Expected dict obs with 'policy' key"
    assert obs["policy"].ndim == 2, "Expected policy obs to be (num_envs, obs_dim)"
    # quick sanity checks for Allegro (16 DoF)
    robot = env.unwrapped.scene["robot"]
    assert len(robot.data.joint_names) == 16, "Expected Allegro to have 16 joints"
    breakpoint()

    action_shape = tuple(env.action_space.shape)
    assert action_shape[-1] == 16, f"Expected last action dim=16, got action_space.shape={action_shape}"
    # initialize EMA prev action from current joint positions
    policy.set_prev_action(robot.data.joint_pos)
    with torch.inference_mode():
        actions = policy(obs)
    assert actions.shape[1] == 16, "Expected 16-dim policy output"
