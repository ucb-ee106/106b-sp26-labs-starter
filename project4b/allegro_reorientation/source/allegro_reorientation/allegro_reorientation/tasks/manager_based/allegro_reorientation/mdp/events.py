...
class reset_joints_within_limits_range(ManagerTermBase):
    ...
    * If the operation is "scale", the values are the scaling factors for the joint limits, i.e. the joint range
    ...
    def _resolve_default_joint_pos_fallback(self, fallback_cfg: dict[str, float] | None) -> torch.Tensor:
        joint_pos = torch.zeros(self._asset.num_joints, device=self._asset.device)
        if fallback_cfg is None:
            return joint_pos

        for joint_name, joint_value in fallback_cfg.items():
            joint_ids = self._asset.find_joints(joint_name)[0]
            joint_pos[joint_ids] = joint_value
        return joint_pos
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        self._default_joint_pos_fallback = self._resolve_default_joint_pos_fallback(
            cfg.params.get("default_joint_pos_fallback")
        )
        self._default_joint_vel_fallback = torch.zeros(self._asset.num_joints, device=self._asset.device)
        # For some imported custom USDs, default joint buffers can be unavailable during
        # early play-callback resolution. Fall back to preset-specific defaults.
        if self._asset.data.default_joint_pos is None:
            default_joint_pos = self._default_joint_pos_fallback
        else:
            default_joint_pos = self._asset.data.default_joint_pos[0]
        if self._asset.data.default_joint_vel is None:
            default_joint_vel = self._default_joint_vel_fallback
        else:
    ...
    def __call__(
        ...
        default_joint_pos_fallback: dict[str, float] | None = None,
    ):
        # get default joint state
        if self._asset.data.default_joint_pos is None:
            joint_pos = self._default_joint_pos_fallback.expand(len(env_ids), -1).clone()
        else:
            joint_pos = self._asset.data.default_joint_pos[env_ids].clone()
        if self._asset.data.default_joint_vel is None:
            joint_vel = self._default_joint_vel_fallback.expand(len(env_ids), -1).clone()
        else:
